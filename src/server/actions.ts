...
type OperationalRole = UserRole.VENDOR | UserRole.MAINTENANCE_PROVIDER | UserRole.CONCIERGE_AGENT | UserRole.GUEST;

const operationalRoles: readonly OperationalRole[] = [
  UserRole.VENDOR,
...
function isOperationalRole(role: UserRole): role is OperationalRole {
  return operationalRoles.some((operationalRole) => operationalRole === role);
}
...
export async function assignUserRoleAction(formData: FormData) {
  ...
  if (isOperationalRole(role) && target.memberships.length > 0) {
